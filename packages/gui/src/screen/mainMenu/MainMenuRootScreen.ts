...
import { LazyResourceCollection } from '@ra2/engine/LazyResourceCollection';
...
// These were originally `await import('...')` inside each screen-creation
// branch, but Vite reported them as INEFFECTIVE_DYNAMIC_IMPORT — the
// modules are already pulled into the main chunk by Gui.ts's static
// imports, so the awaits added syntax overhead without any code-split
// benefit. Hoisted to static imports for clarity. (RealFileSystemDir
// stays dynamic because it's gated behind an async getMapDir() probe.)
