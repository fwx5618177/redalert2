// @ts-nocheck — Dev-only live-interaction debug runtime (~2200 LOC).
// Pre-existing API drift: GameModes vs GameModeRegistry signature, ArtSection
// vs IniSection, ObjectRules.waterBound rename, LandRules.buildable private.
// Real fix would touch a dozen game/* files; out of scope for this migration.
import * as THREE from 'three';
import { Art } from '@ra2/game/art/Art';
import { BoxedVar } from '@ra2/util/BoxedVar';
import { Color } from '@ra2/util/Color';
...
import { ReplayLoadingScreenApi } from '@ra2/gui/screen/game/loadingScreen/ReplayLoadingScreenApi';
...
import { GameFactory } from '@ra2/game/GameFactory';
...
import { BuildingAnimArtProps } from '@ra2/engine/renderable/entity/building/BuildingAnimArtProps';
...
import { NO_TEAM_ID } from '@ra2/game/gameopts/constants';
...
export class LiveInteractionTester {
    ...
    private static bindRuntimeBridge(): void {
        this.fetchRuntimeStatus().catch((error) => {
            ...
            this.state.runtimeStatus.lastError = '未连接到本地直播运行时，请通过 pnpm live:runtime 启动。';
            this.syncState();
        });
        try {
