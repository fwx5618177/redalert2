// @ts-nocheck — Dev-only debug UI tester. Pre-existing API drift (private
// property access, GameObject path duality from `as GameObject` without
// import, ObjectRules.techLevel/constructionYard renames, AudioVisualRules
// private fields). Real fix would require API alignment that's out of
// scope for the monorepo migration. The file runs at runtime and matches
// the actual API; only the type-check view is stale.
...
import { DamageType } from "@ra2/engine/renderable/entity/building/DamageType";
