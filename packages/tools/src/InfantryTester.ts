...
export class InfantryTester {
    ...
    static selectInfantry(infantryType: string): void {
        ...
        const selectedColor = this.currentInfantryColor?.clone()
            ...
            ?? new Color(255, 0, 0);
        // @ts-expect-error TS2540 — Player.color is read-only in current
        // signature; the tester uses player as a transient setup vehicle
        // and overwrites the field directly. Real game code assigns via
        // PlayerList.assignColor().
